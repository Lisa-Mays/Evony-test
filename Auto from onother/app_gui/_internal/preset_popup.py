...
preset_name = args.preset_name.replace(' ', '_').lower()
...
var.set("rally")  # Set the default selected radio button


ymir_checkbox_texts = ["ymir-1", "ymir-2", "ymir-3", "ymir-4", "ymir-5", "ymir-6"]
cerberus_checkbox_texts = ["juniorcerberus-1", "mediumcerberus-2", "seniorcerberus-3", "legendarycerberus-4"]
...
def save_to_json(checkbox_groups, checkbox_group_names):
    ...
    file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), f'evony/config/{file_name}.json')
...
def save_callback():
    checkbox_groups = [ymir_checkbox, cerberus_checkbox]
